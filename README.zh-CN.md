...
[功能](#核心功能) | [部署](#一键部署) | [架构](#系统架构) | [用法](#使用示例) | [API 接口](#reachrich-api-接口) | [FAQ](#常见问题)
...
RRCLAW 负责 LLM 推理主循环：接收用户指令 → 调度工具获取数据 → 执行回测 → 返回结构化结果。在系统中定位为量化决策调度层，而非消息转发中间件。
...
- **因子挖掘** — core_engine Alpha 因子扫描，滚动窗口优化验证
- **条件选股** — 200+ 因子 DSL 组合筛选（技术面 / 情绪面 / 基本面）
- **多通道接入** — Telegram、飞书、WebChat、REST API，统一调度
...
```
rrclaw/
├── runtime/                  # 核心 LLM 循环 + 基础设施
│   ├── conversation.py       # ConversationRuntime（异步生成器）
│   ├── session.py            # JSONL 持久化 + 自动轮转
│   ├── server.py             # 主服务入口
│   ├── config.py             # 3 层配置合并（YAML → 环境变量 → 默认值）
│   ├── providers/            # LLM 提供商
│   │   ├── router.py         # 前缀路由 + 降级链
│   │   ├── anthropic.py      # Claude
│   │   ├── dashscope.py      # 通义千问
...
│
├── tools/                    # 工具体系
│   ├── registry.py           # 全局注册表（Tier 0/1/2）
│   ├── executor.py           # 并发/串行调度
│   ├── builtin/              # 内置工具（bash, file_ops, market_query）
│   ├── mcp/                  # MCP 服务端/客户端
│   │   └── reachrich_server.py  # A 股行情 MCP
│   └── pyagent/              # PyAgent 集成（71 条命令）
│
├── data_sources/             # 数据接入
...
├── context/                  # 上下文工程
│   ├── engine.py             # 5 层压缩
│   └── memory/               # 3 级记忆（会话 / 用户 / 系统）
│
├── evolution/                # 策略进化
│   ├── engine.py             # 跨会话模式检测
│   ├── gepa_pipeline.py      # GEPA 优化流水线
│   └── pattern_detector.py   # 重复工具链 → 自动技能
│
├── channels/                 # 通道接入
│   ├── gateway.py            # OpenClaw WebSocket
│   ├── acp_runtime.py        # ACP 接管模式
│   └── webhook.py            # HTTP 回调
│
└── deploy.sh                 # 一键部署脚本
```

**代码量**：61 个源文件，约 10,800 行 Python。
