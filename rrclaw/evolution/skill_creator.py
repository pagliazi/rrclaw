...
if TYPE_CHECKING:
    from rrclaw.evolution.pattern_detector import ToolChainPattern
    from rrclaw.evolution.failure_detector import FailurePattern
    from rrclaw.evolution.skill_guard import SkillGuard, TrustLevel
    from rrclaw.tools.hermes.runtime import HermesNativeRuntime

logger = logging.getLogger("rrclaw.evolution.skill_creator")
...
class SkillCreator:
    ...
    SKILLS_DIR = Path.home() / ".rrclaw" / "skills"
    TEMPLATE = """---
name: {name}
...
"""
    ...
    async def create_from_pattern(self, pattern: ToolChainPattern) -> CreatedSkill | None:
        ...
        # Security scan
        if self._guard:
            from rrclaw.evolution.skill_guard import TrustLevel
